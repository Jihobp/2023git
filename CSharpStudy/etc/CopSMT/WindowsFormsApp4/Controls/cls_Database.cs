...
namespace WindowsFormsApp4
{
    class cls_Database
    {
        ...
        public DataTable Get_FIFO_ByMatInOut(string LotNo, string Type)
        {
            //자재입,출고 내역에서 LotNo 선입선출 대상 검색
            // Type == "FIFO" 선입선출 대상 리스트 검색 
            query = $@"DECLARE @ItemCd nvarchar(20)
                        SELECT TOP 1 @ItemCd = Item_Cd 
                          FROM MT_MatIn 
                         WHERE Corp_Cd = '{WrGlobal.Corp_Cd}' 
                           AND MatLot_No = '{LotNo}'

                        SELECT FIFO.*, i.Item_Nm, i.Alias,  i.Grade, mi.In_Date, i.Spec, i.Unit
                        FROM
                        (
	                        SELECT Item_Cd, MatLot_No, sum(In_Qty - Out_Qty) AS Remain_Qty
	                        FROM
	                        (
		                        SELECT Corp_Cd, MatLot_No, Item_Cd, sum(In_Qty) AS In_Qty, 0 AS Out_Qty
		                        FROM MT_MatIn
		                        GROUP BY Corp_Cd, MatLot_No, Item_Cd

		                        UNION

		                        SELECT Corp_Cd, MatLot_No, Item_Cd, 0 AS In_Qty, sum(Out_Qty) AS  Out_Qty
		                        FROM MT_MatOut
		                        GROUP BY Corp_Cd, MatLot_No, Item_Cd
	                        ) InOutList

	                        WHERE Corp_Cd = '{WrGlobal.Corp_Cd}' 
	                        AND InOutList.Item_Cd = @ItemCd ";
            if (Type == "FIFO")
                query += $@"AND MatLot_No < '{LotNo}' ";
            else
                query += $@"AND MatLot_No = '{LotNo}' ";


            query += $@"GROUP BY MatLot_No, Item_Cd
                        ) FIFO
                        JOIN BS_Item i on  i.Item_Cd = FIFO.Item_Cd
                        JOIN MT_MatIn mi on mi.MatLot_No = FIFO.MatLot_No
                        WHERE Remain_Qty > 0
                        ORDER BY Item_Cd, MatLot_No ";
